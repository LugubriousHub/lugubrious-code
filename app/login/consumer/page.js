    ...
    <main className="min-h-screen bg-[#020617] px-6 py-10 text-slate-100 md:px-10 md:py-14">
      <div className="mx-auto flex min-h-[82vh] w-full max-w-xl flex-col justify-center">
        ...
        <section className="surface p-8 md:p-10">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{t.consumerTag}</p>
          <h1 className="mt-3 text-4xl font-semibold">{t.consumerTitle}</h1>
          <p className="mt-3 text-slate-300">{t.consumerSubtitle}</p>

          <form onSubmit={handleSubmit} className="mt-6 grid gap-4">
            <label className="grid gap-1 text-sm text-slate-200">
              {t.email}
              <input type="email" className="field" placeholder="family@email.com" required />
            </label>
            <label className="grid gap-1 text-sm text-slate-200">
              {t.password}
